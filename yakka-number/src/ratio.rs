...
pub trait Ratioable<const N: usize>: private::Sealed {
    ...
    const RATIO: Ratio = const {
        match NonZero::new(N) {
            ...
            None => unreachable!("cannot have a ratio of zero"),
        }
    };
}
...
///
/// This has no meaning on its own, as it is just a number. To present itself with a meaning, it must accompany a directionality in the relation.
///
/// For example, take a 2-tuple `(2, 4)`, which is a ratio of `2:4`. This can be interpreted as `2` being half of `4`, or `4` being twice as much as `2`.
///
/// Therefore, this type is not a ratio in the mathematical sense, but rather a ratio in the physical sense.
...
/// A trait for types that can be transformed into a distinct equivalence through the use of a [`Ratio`].
pub trait Transform {
    ...
    S: Div<S, Output = S>,
{
    ...
    fn up(self: Self, ratio: Ratio) -> Self {
        ...
        let target_value = S::from(target_ratio.get());
