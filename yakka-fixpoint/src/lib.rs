...
///  - The *fractional* part: extracted as-is, but then left-shifted towards MSB as per the defined fractional bits to reach a standard floating-point representation.
...
pub struct Partwise {
    ...
    ///
    /// The sign is contained in the [`Partwise::integer`] field, so this does not require a signed type.
    fractional: u64,
}
...
pub trait Floating {
    ...
    /// Convert a floating-point number into a [`i64`] representing its integer part.
