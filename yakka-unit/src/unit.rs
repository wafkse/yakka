...
pub trait Unit {
    ...
    /// Convert a value measured in the current unit to one measured in another unit of the same dimension.
    #[inline]
    fn to<U>(self: Self) -> U
    where
    ...
    /// Convert a value measured in another unit of the same dimension to one measured in the current unit.
    #[inline]
