//! Angular motion.
//!
//! This module provides a representation of angular motion in 2-dimensional and 3-dimensional
//! space.
//!
...
///   - Unique representation of angles, i.e: `256` does not fit in an [`u8`] but `2π rad` is
///     exactly the same as `0 rad`.
///   - Precision of `1 / 128` of `π rad` is sufficient for most applications, more specifically, we
///     have up to `(1 / 128) × pi radians ≈ 1.406250000°` degrees of precision.
...
where
    T: Scalar,
{
    /// Instantiate a new [`Angle`] from an [`u8`] representing the angle in counts of `1 / 128` of
    /// `π rad`.
    #[inline]
