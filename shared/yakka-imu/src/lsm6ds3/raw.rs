...
use yakka_bit::state::State;

use crate::lsm6ds3::register::{Read, Write, file::Ctrl3C};
...
pub struct Behavior {
    ...
    pub fifo_settings: FifoSettings,
}

/// An error during [`RawLsm6ds3`] operation.
#[derive(Debug, Clone, Copy, Eq, PartialEq, PartialOrd, Ord, Hash)]
pub enum Error<E> {
...
/// A raw handle to a [`Lsm6ds3`] inertial measurement unit.
///
/// This exposes the raw sensing capabilities of the device, without further processing.
#[derive(Debug)]
pub struct RawLsm6ds3<'a, I, P, D, A = SevenBitAddress>(&'a mut Lsm6ds3<I, P, D, A>)
where
    I: I2c<A>,
    P: Wait,
    D: DelayNs,
...
impl<'a, I, P, D, A> RawLsm6ds3<'a, I, P, D, A>
where
    I: I2c<A>,
    P: Wait,
    D: DelayNs,
    A: Address,
{
    /// Initialize a new [`RawLsm6ds3`] device handle.
    ///
    /// This will completely reset the device, default to its preset settings.
    #[inline]
    pub async fn handle(
        target_device: &'a mut Lsm6ds3<I, P, D, A>,
    ) -> Result<Self, Error<I::Error>> {
        const RESET_DELAY_NS: u32 = 100;

        let control_value = Ctrl3C::read(target_device.channel_mut()).await?;

        Ctrl3C::write(
            target_device.channel_mut(),
            control_value.sw_reset().enabled(),
        )
        .await?;
        ...
        loop {
            ...
            match control_value.sw_reset().state() {
                State::Set => target_device.timer_mut().delay_ns(RESET_DELAY_NS).await,
                State::Cleared => {
                    break Ctrl3C::write(
                        target_device.channel_mut(),
                        control_value
...
impl<'a, I, P, D, A> Deref for RawLsm6ds3<'a, I, P, D, A>
where
    I: I2c<A>,
    P: Wait,
    D: DelayNs,
...
impl<'a, I, P, D, A> DerefMut for RawLsm6ds3<'a, I, P, D, A>
where
    I: I2c<A>,
    P: Wait,
    D: DelayNs,
